...
def get_matriz_riesgos():
    ...
    try:
        ...
        # Log para debug
        print(f"Total de filas obtenidas de la consulta: {len(rows)}")
        if len(rows) > 0:
            print(f"Primera fila: activo_id={rows[0].id_activo}, probabilidad={rows[0].probabilidad}, impacto={rows[0].impacto}")
        ...
        for row in rows:
            ...
            if not probabilidad or not impacto:
                print(f"Advertencia: Activo {activo_id} ({row.nombre_activo}) tiene probabilidad o impacto NULL, omitiendo de matriz")
                continue
            ...
            if prob_idx is None:
                ...
                for key, idx in probabilidades_map.items():
                    if key.lower() == prob_lower:
                        ...
                        break
                if prob_idx is None:
                    print(f"Advertencia: Probabilidad '{probabilidad}' no encontrada en mapeo para activo {activo_id}, usando 'Posible' (índice 3)")
            ...
            if imp_idx is None:
                ...
                for key, idx in impactos_map.items():
                    if key.lower() == imp_lower:
                        ...
                        break
                if imp_idx is None:
                    print(f"Advertencia: Impacto '{impacto}' no encontrado en mapeo para activo {activo_id}, usando 'Menor' (índice 1)")
                    imp_idx = 1  # Usar 'Menor' como default
            
            # Validar índices
            if prob_idx < 0 or prob_idx >= 5 or imp_idx < 0 or imp_idx >= 5:
                print(f"Advertencia: Índices fuera de rango para activo {activo_id}: prob_idx={prob_idx}, imp_idx={imp_idx}")
                continue
        ...
        # Log para debug
        print(f"Total de activos únicos procesados: {total_activos_evaluados}")
        print(f"Activos en activos_data: {list(activos_data.keys())}")
        ...
        print(f"Suma de celdas en matriz: {suma_celdas}")
        ...
        print(f"Error obteniendo matriz de riesgos: {str(e)}")
        import traceback
        traceback.print_exc()
...
def get_reporte_completo_riesgos():
    ...
    try:
                ...
                if row.id_evaluacion_riesgo_activo:
                    try:
                        controles_sql = text("""
                            ...
                            JOIN controles_seguridad c ON rca.ID_Control = c.ID_Control
                        ...
                        print(f"Error obteniendo controles para evaluación {row.id_evaluacion_riesgo_activo}: {str(ctrl_error)}")
                        controles_result = []
                    controles_list = [{
                        'id': ctrl.ID_Control,
                        'nombre': ctrl.Nombre,
        ...
        try:
            ...
            print(f"Error obteniendo activos faltantes: {str(e)}")
        ...
        try:
            ...
            print(f"Error calculando estadísticas de riesgos: {str(e)}")
            import traceback
            traceback.print_exc()
        ...
        import traceback
        error_trace = traceback.format_exc()
        print(f"Error en get_reporte_completo_riesgos: {str(e)}")
        print(f"Traceback: {error_trace}")
        # Retornar estructura vacía en lugar de error para evitar que la UI se rompa
        return jsonify({
            'activos': [],
            'estadisticas': {
                'total_activos': 0,
...
def get_activos_evaluados_detalle():
    ...
    try:
        ...
        import traceback
        error_trace = traceback.format_exc()
        print(f"Error en get_activos_evaluados_detalle: {str(e)}")
        print(f"Traceback: {error_trace}")
        return jsonify({'error': str(e)}), 500
    ...
    try:
        ...
        for row in result:
            ...
            try:
                ...
                print(f"Error en query de controles: {e}")
                controles_sql = None
        ...
        import traceback
        error_trace = traceback.format_exc()
        print(f"Error en get_riesgos_altos_detalle: {str(e)}")
        print(f"Traceback: {error_trace}")
        return jsonify({'error': str(e)}), 500
...
def get_riesgos_con_activos():
    ...
    try:
        from sqlalchemy import text
        from ..models import Riesgo, Activo, evaluacion_riesgo_activo, controles_seguridad
        ...
        for riesgo_id, riesgo_data in riesgos_dict.items():
            try:
                ...
                if not tipo_riesgo:
                    ...
                    controles_sql = text("""
                        SELECT 
                            ...
                            c.categoria_control_iso
                        FROM controles_seguridad c
                        WHERE c.Categoria LIKE :tipo_pattern
                ...
                print(f"Error obteniendo controles sugeridos para riesgo {riesgo_id}: {ctrl_error}")
        ...
        import traceback
        error_trace = traceback.format_exc()
        print(f"Error en get_riesgos_con_activos: {str(e)}")
        print(f"Traceback: {error_trace}")
        return jsonify({'error': str(e)}), 500
