...
const InformeRiesgos: React.FC = () => {
  ...
  const datosGrafico = React.useMemo(() => {
    ...
    // Ordenar por cantidad de activos (descendente) para mejor visualización
  ...
  const exportarPDF = async () => {
    ...
    try {
      ...
      if (reporteData.activos && reporteData.activos.length > 0) {
        reporteData.activos.forEach((activo: any, index: number) => {
          ...
          if (activo.descripcion) {
            const descLines = doc.splitTextToSize(`Descripción: ${activo.descripcion}`, maxWidth);
          ...
          if (activo.riesgos && activo.riesgos.length > 0) {
            ...
            activo.riesgos.forEach((riesgo: any, riesgoIndex: number) => {
              ...
              if (riesgo.evaluacion_inherente) {
                doc.text(`  Evaluación Inherente:`, margin + 5, yPos);
                yPos += 5;
                ...
                if (riesgo.evaluacion_inherente.justificacion) {
                  const justLines = doc.splitTextToSize(`    Justificación: ${riesgo.evaluacion_inherente.justificacion}`, maxWidth - 15);
              ...
              if (riesgo.evaluacion_residual) {
                doc.text(`  Evaluación Residual:`, margin + 5, yPos);
                yPos += 5;
                ...
                if (riesgo.evaluacion_residual.justificacion) {
                  const justLines = doc.splitTextToSize(`    Justificación: ${riesgo.evaluacion_residual.justificacion}`, maxWidth - 15);
                  doc.text(justLines, margin + 10, yPos);
                  yPos += justLines.length * 5;
                }
              }
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        doc.text(
          `Página ${i} de ${totalPages} - SGRI Sistema de Gestión de Riesgos de Información`,
          pageWidth / 2,
    ...
    try {
      ...
      const consolidadoData = [
        ['ID Activo', 'Nombre Activo', 'Tipo', 'Criticidad', 'Estado', 'ID Riesgo', 'Nombre Riesgo', 
         'Categoría', 'Prob. Inh.', 'Impacto Inh.', 'Nivel Inh.', 'Prob. Res.', 'Impacto Res.', 
         'Nivel Res.', 'Justificación Inh.', 'Justificación Res.', 'Total Controles']
      ];
      ...
      reporteData.activos?.forEach((activo: any) => {
        ...
        detalleData.push(['Descripción', activo.descripcion || '']);
        detalleData.push(['']);
        
        if (activo.riesgos && activo.riesgos.length > 0) {
          detalleData.push(['RIESGOS IDENTIFICADOS']);
          detalleData.push(['Nombre', 'Categoría', 'Prob. Inh.', 'Impacto Inh.', 'Nivel Inh.', 
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 4 }}>
        <Box>
          ...
          <Typography variant="body1" className="font-roboto" sx={{ color: '#6B7280' }}>
            Análisis completo de activos y riesgos identificados
          </Typography>
        </Box>
              ...
              {datosNivelRiesgo.length > 0 && datosNivelRiesgo.some(d => d.cantidad > 0) ? (
                  ...
                  <PieChart>
                    <Pie
                      ...
                      label={({ nivel, cantidad, percent }) => 
                        `${nivel}: ${cantidad} (${(percent * 100).toFixed(1)}%)`
                      }
                      outerRadius={120}
                      fill="#8884d8"
                      dataKey="cantidad"
                    ...
                    <Legend 
                      ...
                      payload={datosNivelRiesgo.map((entry, index) => ({
                        value: entry.nivel,
                        type: 'square',
                        id: entry.nivel,
                        color: entry.color,
                ...
                <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: 400 }}>
                  <Typography variant="body2" sx={{ color: '#6B7280' }}>
                    No hay datos de distribución de riesgos disponibles
                  </Typography>
                </Box>
              )}
            </CardContent>
                      ...
                      <Box>
                        ...
                        <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', mt: 1 }}>
                          ...
                          <Chip 
                            ...
                            size="small"
                            sx={{
                              backgroundColor: activo.criticidad === 'Alto' || activo.criticidad === 'Crítico' ? '#FEE2E2' : '#D1FAE5',
                              color: activo.criticidad === 'Alto' || activo.criticidad === 'Crítico' ? '#DC2626' : '#065F46'
                            }}
                          />
      ...
      >
        <DialogTitle sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Typography variant="h6" className="font-poppins" sx={{ color: '#1E3A8A', fontWeight: 600 }}>
            Riesgos Altos - Estado de Mitigación ({dataModalRiesgosAltos?.total || 0})
          </Typography>
            ...
            <Stack spacing={3}>
              {dataModalRiesgosAltos.riesgos.map((riesgo: any, index: number) => (
                <Accordion key={riesgo.id_riesgo || index} defaultExpanded={index === 0}>
                  ...
                  <AccordionDetails>
                    <Grid container spacing={3}>
                      {/* Columna izquierda: Información del Riesgo y Responsable */}
                      <Grid item xs={12} md={4}>
                        <Paper sx={{ p: 2, backgroundColor: '#F9FAFB', height: '100%' }}>
                          <Typography variant="subtitle1" sx={{ fontWeight: 600, mb: 2, color: '#1E3A8A' }}>
                                ...
                                {riesgo.documentos_evidencia.map((documento: any, docIndex: number) => (
                                  ...
                                  >
                                    ...
                                    <ListItemText
                                      ...
                                      secondary={
                                        <Typography variant="caption" sx={{ color: '#6B7280' }}>
                                          {documento.tamaño && `${documento.tamaño} • `}
                    ...
                    <TableRow key={activo.id || activo.ID_Activo || index} hover>
                      ...
                      <TableCell>
                        <Chip 
                          ...
                          size="small"
                          sx={{
                            backgroundColor: (activo.criticidad || activo.nivel_criticidad_negocio) === 'Alto' || (activo.criticidad || activo.nivel_criticidad_negocio) === 'Crítico' ? '#FEE2E2' : '#D1FAE5',
                            color: (activo.criticidad || activo.nivel_criticidad_negocio) === 'Alto' || (activo.criticidad || activo.nivel_criticidad_negocio) === 'Crítico' ? '#DC2626' : '#065F46'
                          }}
                        />
                      </TableCell>
                      <TableCell>
