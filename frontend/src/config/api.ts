...
const getApiBaseUrl = (): string => {
  // Si hay una variable de entorno definida, úsala (tiene prioridad)
  ...
  // usando el hostname actual (que será la IP del servidor)
  const protocol = window.location.protocol;
  const hostname = window.location.hostname;
  const port = window.location.port || '5173'; // Si no hay puerto, asumir 5173
  
  // Si estamos en el puerto 5173 (frontend), el backend está en 5000
  if (port === '5173' || port === '') {
