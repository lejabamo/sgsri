...
const DocumentManager: React.FC<DocumentManagerProps> = ({
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (!documentosService.validarTipoArchivo(file)) {
        setError('Tipo de archivo no permitido. Solo se permiten PDF, Word, Excel, imágenes y archivos de texto.');
        return;
      }

      // Validar tamaño
      if (!documentosService.validarTamañoArchivo(file)) {
  ...
  const handleSubirDocumento = async () => {
    ...
    try {
      // Simular subida de documento (en producción se conectaría con el backend)
      const nuevoDocumento: DocumentoAdjunto = {
        id: Date.now().toString(),
        nombre: archivoSeleccionado.name,
        tipo: archivoSeleccionado.type,
        tamaño: archivoSeleccionado.size,
        url: URL.createObjectURL(archivoSeleccionado), // En producción sería la URL real
  ...
  return (
    <Box>
      ...
      {/* Dialog para agregar descripción */}
        ...
        <DialogContent>
          <Box sx={{ mt: 2 }}>
            ...
            <TextField
              ...
              placeholder="Describe el contenido o propósito de este documento..."
              sx={{ borderRadius: '8px' }}
            />
          </Box>
        </DialogContent>
