...
interface ResidualJustificationSuggestion {
  ...
  articulo: string;
  confianza: number;
  relacion_inherente: string; // Explica la relación con el riesgo inherente
...
const ResidualJustificationSuggestions: React.FC<ResidualJustificationSuggestionsProps> = ({
  ...
  const loadJustificationSuggestions = async () => {
    if (!residualRisk.probabilidad || !residualRisk.impacto) {
      return; // No cargar si no hay evaluación residual
    }
  ...
  const generateFallbackSuggestions = (): ResidualJustificationSuggestion[] => {
    ...
    if (inherentRisk.probabilidad !== residualRisk.probabilidad) {
      suggestions.push({
        ...
        descripcion: `Los controles implementados han reducido la probabilidad de ${inherentRisk.probabilidad} a ${residualRisk.probabilidad}. Esta reducción se debe a la eficacia de los controles de seguridad aplicados.`,
        norma: 'ISO 27005',
        articulo: 'A.6.1.2',
        confianza: 0.85,
        relacion_inherente: `El riesgo inherente tenía una probabilidad ${inherentRisk.probabilidad}, que ha sido mitigada a ${residualRisk.probabilidad} mediante los controles implementados.`,
        controles_mencionados: selectedControls.slice(0, 3)
      });
    }

    // Sugerencia 2: Reducción de impacto
    if (inherentRisk.impacto !== residualRisk.impacto) {
      suggestions.push({
        id: '2',
        titulo: 'Mitigación de Impacto',
        descripcion: `El impacto se ha reducido de ${inherentRisk.impacto} a ${residualRisk.impacto} gracias a los controles de protección y recuperación implementados.`,
        norma: 'ISO 27002',
        articulo: 'A.12.3',
        confianza: 0.80,
        relacion_inherente: `El impacto inherente era ${inherentRisk.impacto}, ahora es ${residualRisk.impacto} debido a las medidas de mitigación.`,
    ...
    if (inherentLevel !== residualLevel) {
      ...
      suggestions.push({
        ...
        descripcion: `El nivel de riesgo se ha reducido de ${nivelMap[inherentLevel] || inherentLevel} a ${nivelMap[residualLevel] || residualLevel}. Los controles implementados han demostrado su eficacia en la mitigación del riesgo.`,
        norma: 'ISO 27005',
        articulo: 'A.8.1',
        confianza: 0.90,
    ...
    if (selectedControls.length > 0) {
      suggestions.push({
        ...
        descripcion: `Los controles seleccionados (${selectedControls.slice(0, 3).join(', ')}) han demostrado eficacia en la reducción del riesgo. Según ISO 27005, la evaluación residual debe considerar la efectividad real de los controles implementados.`,
        norma: 'ISO 27005',
        articulo: 'A.8.2',
        confianza: 0.75,
  ...
  const handleAccept = (suggestion: ResidualJustificationSuggestion) => {
    if (onJustificationSelect) {
      // Construir justificación completa con relación al inherente
    ...
    return (
      <Alert severity="info" sx={{ borderRadius: '8px' }}>
        <Typography variant="body2">
          Completa la evaluación residual (probabilidad e impacto) para ver sugerencias de justificación basadas en normativa.
        </Typography>
      </Alert>
    );
  }
    ...
    <Box>
      <Box display="flex" alignItems="center" justifyContent="space-between" mb={2}>
        <Box display="flex" alignItems="center">
          ...
          <Typography variant="h6" sx={{ color: '#1E3A8A', fontWeight: 'bold' }}>
            💡 Justificaciones ISO 27005
          </Typography>
        </Box>
        <Tooltip title="Las sugerencias están basadas en normativa ISO 27005 y consideran la relación con el riesgo inherente evaluado">
      ...
      {acceptedSuggestions.size > 0 && (
        <Alert severity="success" sx={{ mt: 2, borderRadius: '8px' }}>
          <Typography variant="body2">
            <strong>✓ Justificación cargada.</strong> Puedes complementar o editar el texto en el campo de justificación.
          </Typography>
        </Alert>
      )}
    </Box>
